...
void mouseBreakBlock(struct GameData* gameData, int xMouseCor, int yMouseCor){
    //Cords calculations
...
void cameraControlInput(struct GameData* gameData, SDL_Event event){
    ...
    if (!mouseOnInMenuWindow) {
        ...
        if (event.type == SDL_MOUSEBUTTONDOWN) {
            mouseBreakBlock(gameData, xCor, yCor);
        }
    }

    float camSpeed = 10;
    int timeAdjustedCamChange = gameData->screen->frameRenderTime * camSpeed;
