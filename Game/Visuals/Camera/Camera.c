...
void renderChunkTexture(struct GameData* gameData, struct CastedChunk* castedChunk){
    ...
    for (int x = 0; x < gameData->cameraData->chunksScale; x++){
        for (int y = 0; y < gameData->cameraData->chunksScale; y++){
            ...
            SDL_Rect rightBLock = {cords[0] + (cameraData->renderScale/2), cords[1], cameraData->renderScale, cameraData->renderScale};
            SDL_Rect leftBlock = {cords[0], cords[1], cameraData->renderScale,cameraData->renderScale};
...
void renderView(struct GameData* gameData, int xCor, int yCor){
    ...
    int xChunkTextureRez = cameraData->chunkPixelScale;
    int yChunkTextureRez = cameraData->chunkPixelScale/2;
    ...
    for (int x = 0; x < cameraData->viewDistance; x++){
        for (int y = 0; y < cameraData->viewDistance; y++){
            ...
            SDL_Rect chunkRect = {cords[0] + xCor + cameraData->xRenderingOffset, cords[1] + yCor + cameraData->yRenderingOffset, xChunkTextureRez, yChunkTextureRez};
